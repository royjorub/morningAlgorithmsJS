...
function partition(nums = [], left = 0, right = nums.length - 1) {
    ...
    var pivotInd = Math.floor(left + ((right - left) / 2));
    while (left < pivotInd && nums[left] <= nums[pivotInd]){
        left ++;
    }
    while (right > pivotInd && nums[pivotInd < nums[right]]) {
        right --;
    }
    let tmp = nums[left];
