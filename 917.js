...
const user = {
  ...
  function lens(subject, path) {
    if (path.length === 0) {
        return subject;
    }
  
    let i = 0;
    while (i < path.length && subject !== null && subject !== undefined) {
        subject = subject[path[i]];
        i++;
    }
    // i >= path.length || subject === null || subject === undefined
  
    return (i < path.length) ? null : subject;
  }
  ...
  */
  ...
  function getMaxServings(recipe, available) {
    const minIngredientServing = Infinity
    for (const ingredient in recipe){
      if (ingredient in available){
        const servingsPerIng = Math.floor(available[ingredient] / recipe[ingredient])
        if (servingsPerIng < minIngredientServing){
          minIngredientServing = servingsPerIng
        }
      }
      else return 0
    }
    return minIngredientServing
